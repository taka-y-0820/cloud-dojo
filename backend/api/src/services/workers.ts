...
import { connection, DockerBuildTask, DockerRunTask, ComposeUpTask, ComposeDownTask, K8sDeployTask, CICDRunTask } from './queue';
...
// Initialize workers
export function initializeWorkers(fastify?: FastifyInstance) {
  if (fastify) {
  ...
  composeService.on('update', (session) => {
    ...
    completedServices = services.filter(s => s.status === 'running' || s.status === 'error').length;
    
    const progress = totalServices > 0 ? Math.round((completedServices / totalServices) * 90) + 10 : 10;
